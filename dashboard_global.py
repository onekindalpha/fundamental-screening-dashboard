#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Global/Asia Peter Lynch + Graham Dashboard - US-style layout"""
...
st.set_page_config(
    page_title="🌏 Global/Asia Lynch-Graham Dashboard",
    page_icon="🌏",
    layout="wide",
...
UNIVERSE_FILES = {
    # Global/Asia 고정 universe. 파일명 변형을 어느 정도 허용한다.
    "Taiwan 50": ["results_global/taiwan50_screening_*.tsv", "results_global/*taiwan*50*.tsv"],
    "Japan Nikkei 225": ["results_global/japan_nikkei225_screening_*.tsv", "results_global/*nikkei*225*.tsv", "results_global/*nikkei*.tsv"],
    "China / Hong Kong": ["results_global/china_hk_screening_*.tsv", "results_global/*china*hk*.tsv", "results_global/*hong*kong*.tsv"],
    "India Nifty 50": ["results_global/india_nifty50_screening_*.tsv", "results_global/*nifty*50*.tsv", "results_global/*nifty*.tsv"],
    "Global Semicon / PCB / Memory / OSAT": [
        "results_global/global_semicon_pcb_memory_osat_screening_*.tsv",
        "results_global/*semicon*pcb*.tsv",
        "results_global/*memory*osat*.tsv",
    ],
}
...
PROFILE_PATH = Path("results_global/global_company_profiles.csv")
...
def link_buttons(ticker: str, name: str = "", cik: str = "") -> None:
    """Global-friendly external links for the selected ticker.

    Keep this lightweight: global tickers do not always have SEC/CIK links,
    so the default links are Yahoo Finance and Google Finance/search.
    """
    ...
    yahoo_url = f"https://finance.yahoo.com/quote/{quote_plus(ticker)}"
    google_finance_url = f"https://www.google.com/finance/quote/{quote_plus(ticker)}"
    search_url = "https://www.google.com/search?q=" + quote_plus(f"{ticker} {name} investor relations")

    cols = st.columns(3)
    cols[0].link_button("Yahoo Finance", yahoo_url)
    cols[1].link_button("Google Finance", google_finance_url)
    cols[2].link_button("IR / 회사 검색", search_url)
...
def available_result_files() -> list[str]:
    return sorted([str(p) for p in Path("results_global").glob("*.tsv")])
...
def load_universe(name: str) -> tuple[pd.DataFrame | None, str | None]:
    ...
    #    results_global/*.tsv 전체에서 유니버스 컬럼이 해당 이름인 행을 복구한다.
    frames = []
    sources = []
...
def latest_update_time(universe: str, source: str | None) -> str:
    ...
    if source:
        ...
        if src and Path(src).exists():
            ...
            p = Path("results_global") / Path(src).name
            if p.exists():
                paths.append(p)
...
def render_global_interpretation_notes(compact: bool = False) -> None:
    """Global/Asia-specific interpretation notes, using the same placement style as the US dashboard."""
    body = """
<div class="explain-box">
<b>글로벌/아시아 데이터 해석 메모</b><br>
대만·일본·중국/홍콩·인도 종목은 <b>시장, 통화, 회계 기준, 재무 데이터 커버리지</b>가 다릅니다.
같은 표에서 보되, <b>시장 / 통화 / 데이터상태</b>를 같이 확인합니다.
<br><br>
<b>단기위험부채</b><br>
가능하면 Short-term debt, Current debt, Current portion of long-term debt 계열을 묶어 봅니다.
다만 글로벌 데이터는 항목 누락이 있을 수 있으므로, 이 필터는 린치 관점의 보조 확인용입니다.
<br><br>
<b>PCB·기판·메모리·후공정(OSAT)</b>
<ul>
<li>사이클 업종입니다. EPS 성장률과 PEG가 좋아 보여도 업황 피크 여부, 고객사 CAPEX, 판가 인상 지속성을 같이 봐야 합니다.</li>
<li>일본 전자부품/장비, 대만 기판·후공정, 중국/HK 소재·CCL, 인도 대형주는 서로 통화와 회계 기준이 다릅니다.</li>
<li>보조 추세지표는 Lynch/Graham 판정에 섞지 않고, 시장 반응 확인용으로만 봅니다.</li>
<li>데이터상태가 일부부족/핵심부족이면 결측 항목을 숨기지 않고 표에서 확인합니다.</li>
</ul>
</div>
"""
    if compact:
        ...
        with st.expander("📘 글로벌/아시아 업종별 해석 메모", expanded=True):
            st.markdown(body, unsafe_allow_html=True)
...
def render_detail(row: pd.Series) -> None:
    ...
    t6.metric("40일선 상회", str(row.get("40일선상회(보조)", "-") or "-"))
    t7.metric("200일선 상회", str(row.get("200일선상회(보조)", "-") or "-"))
    t8.metric("200일 이평", fmt_num(row.get("200일이평(보조)")))

    render_global_interpretation_notes(compact=False)
    ...
    cols = [
        ...
        "52주고점대비(보조,%)", "40일이평(보조)", "200일이평(보조)", "40일선상회(보조)", "200일선상회(보조)",
...
def main():
    st.title("🌏 Global / Asia Peter Lynch + Graham Dashboard")
    st.caption("Taiwan 50 / Japan Nikkei 225 / China-Hong Kong / India Nifty 50 / Global Semicon·PCB·Memory·OSAT")
...
각 Universe는 별도 결과 파일을 읽습니다.<br>시장별 통화와 데이터상태를 함께 확인합니다.
</div>
""",
            unsafe_allow_html=True,
        )
...
""",
        ...
        with st.expander("글로벌/아시아 업종별 해석", expanded=False):
            render_global_interpretation_notes(compact=True)

    df, source = load_universe(universe)
    if df is None or len(df) == 0:
        st.warning("선택한 Global/Asia Universe의 결과 파일을 찾지 못했습니다. results_global 안의 파일명도 함께 확인합니다.")
        files = available_result_files()
        if files:
            st.caption("현재 감지된 results_global TSV 파일")
            st.code("\n".join(Path(x).name for x in files), language="text")
        else:
            st.code(
                "python -u build_global_universe_files.py && ./run_global_universes.sh",
                language="bash",
            )
        return
    ...
    for state_key, valid_options in [
        ("global_total_filter", total_options),
        ("global_peg_filter", peg_options),
        ("global_pegy_filter", pegy_options),
    ]:
    ...
    st.info(
        ...
        f"판정필터: 종합={filter_label(total_filter)}, PEG={filter_label(peg_filter)}, PEGY={filter_label(pegy_filter)} / "
        f"데이터 {data_label}"
    )
    ...
    edited = st.data_editor(
        ...
        key=f"global_table_{universe}_{eps_basis}_{graham_basis}_{sort_mode}_{filter_label(total_filter)}_{filter_label(peg_filter)}_{filter_label(pegy_filter)}",
    )
